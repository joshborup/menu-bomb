...
SELECT orders.*, order_items.id as order_item_table_id, order_items.order_id, order_items.name, order_items.price, order_items.description, order_items.image_url, order_items.category, order_items.quantity, order_items.notes, users.first_name, users.last_name, users.phone FROM orders
JOIN order_items ON(orders.id = order_items.order_id)
